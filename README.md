...
| **Capacity tester** | A built-in system speed test (Capacity tab): six fixed real-world agent flows (support, document intelligence, coding fix-loop, research, ops analyst, deep multi-agent) — every one with tool round-trips, compounding context, and realistic sessions — ramp whole reference tiles (a fixed weighted 'Agent Capacity Unit' — comparable across runs; custom mixes flagged non-comparable) against the local SGLang engine, a simulated remote (bell-curve latency), a real cloud endpoint, or — the truth test — the real orchestrator end-to-end (complete planner/worker/validation workflows, measured in workflows/hour with per-run request traces for calibrating the synthetic profiles). Capacity = the last rung where EVERY profile met its own baseline-relative p95 + error SLO; the breach names the profile that failed the next rung. Live CPU/memory/memory-bandwidth/KV/power telemetry, per-agent-type KV footprint, a speed-test-style result, and DB-persisted benchmark history with side-by-side compare (non-comparability warnings), labels, and JSON export (`/capacity/*`, `/capacity/history`) |
