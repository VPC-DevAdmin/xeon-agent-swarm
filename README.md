...
| **Capacity tester** | A built-in VMmark-style benchmark with two separately measured points. **Service capability** is the largest whole-tile concurrent-session level where every workflow type meets its fixed deadline under a joint 95% confidence rule. **Sustainable capacity** is the conservative clean-workflow rate at the queue knee under independently scheduled open-loop arrivals, confirmed in two disjoint windows at the same rate. Agent-host, integrated-node, and inference-diagnostic boundaries remain explicit. Resource, dollar, duration, and generator stops are lower-bound censoring events; interference and harness/workload-integrity failures cannot produce reportable capacity. Cloud presets auto-fill public token prices, and every cloud path uses the dollar circuit breaker. Reference tiles are comparable; custom mixes are diagnostic. See [benchmark methodology v8](docs/benchmark-methodology-v8.md). History supports compare, labels, and JSON export (`/capacity/*`, `/capacity/history`). |
