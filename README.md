...
| **Capacity tester** | A built-in VMmark-style speed test with two explicit dimensions. **Agent host capacity** runs complete planner/worker/validation workflows on this orchestrator while inference calls a remote mock or selected cloud model. **Integrated agent node capacity** runs those same workflows with every model role pinned to the local SGLang engine. **Inference engine diagnostic** bypasses orchestration and sends six synthetic agent-shaped traces directly to local, mock, or cloud inference. Reference tiles are comparable; custom mixes are not. Capacity is the last rung where every profile met its own baseline-relative p95 and error SLO. The ramp has no session or duration ceiling: it continues to SLO, error, CPU, RAM, or KV saturation (a throughput plateau is recorded as the efficiency knee but never ends the ramp — capacity is the SLO boundary, not the knee). Cloud runs require an explicit dollar circuit breaker; hitting it is reported as an incomplete lower-bound run, never as measured saturation. Cloud presets auto-fill public token prices and results show incremental, cumulative, and steady-state cost by capacity level. History supports compare, labels, and JSON export (`/capacity/*`, `/capacity/history`). |
