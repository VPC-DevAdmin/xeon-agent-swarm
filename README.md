...
| **Tool catalog** | A curated set of 23 tools/connectors (messaging, social, data stores, web, dev) users configure in the Tools gallery; the planner sees the enabled ones (`config/tool_catalog.yaml`) and routes tool-using subtasks to the `tool_user` worker. Crowd-pleasers (CSV, SQLite, Telegram, SMS, email, X-read) execute for real; the rest are catalogued + configurable. Catalog at `GET /tools`, per-role grants at `GET /toolbox` |
