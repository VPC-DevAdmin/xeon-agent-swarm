...
| **Agent definitions** | Persistent configured agents (`/agent-definitions`): instructions, tool grants, plan-approval gate, budgets, session policy, optional schedule (kept in sync with a Job). Versioned with history snapshots, clonable, run-once test console, and assignable into the capacity benchmark's e2e planning mix. Terminology: *definition* = the persistent agent, *run* = one execution, *worker* = runtime specialist subagent, *workload profile* = benchmark trace, *virtual session* = load-generator actor, *tile* = fixed workload bundle |
| **Capacity tester** | A built-in VMmark-style speed test with two explicit dimensions. **Agent host capacity** runs complete planner/worker/validation workflows on this orchestrator while inference calls a remote mock or selected cloud model. **Integrated agent node capacity** runs those same workflows with every model role pinned to the local SGLang engine. **Inference engine diagnostic** bypasses orchestration and sends six synthetic agent-shaped traces directly to local, mock, or cloud inference. Reference tiles are comparable; custom mixes are not. Capacity is the STABILITY boundary: sessions are introduced one at a time along the tile rotation and certified at whole-tile boundaries; a level certifies when latency is stationary at fixed load (the window's second half within tolerance of the first) and every profile's error rate stays bounded. The ramp has no session or duration ceiling: it continues until latency climbs at fixed load (`unstable`), errors cascade, or CPU/RAM/KV saturate. The throughput plateau is recorded as the efficiency knee (diagnostic, never a stop), and the default 3×-baseline latency budget is reported as an overlay (`slo_capacity`) so buyers can apply their own budget to the curve. Cloud runs require an explicit dollar circuit breaker; hitting it is reported as an incomplete lower-bound run, never as measured saturation. Cloud presets auto-fill public token prices and results show incremental, cumulative, and steady-state cost by capacity level. History supports compare, labels, and JSON export (`/capacity/*`, `/capacity/history`). |
...
| `OPENAI_API_KEY` / `ANTHROPIC_API_KEY` / `GOOGLE_API_KEY` | optional server-side keys for cloud model presets; a one-run key can instead be entered in the Capacity UI and is never persisted |
