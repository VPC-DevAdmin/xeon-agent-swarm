...
def test_call_key_names_archetype_role_and_phase():
    ...
    assert mr.call_key(_body("You are a research specialist.", "Research: Using ONLY the build available")) == "code_agent/research/0"
    assert mr.call_key(_body("You are a research specialist.", "Research: Using ONLY the dataset (L) available",
                             extra=[{"role": "tool", "content": "x", "tool_call_id": "1"}])) == "analyst_large/research/1"
    assert mr.call_key(_body("You are the output validator.", "Using ONLY the field notes below")) == "deep_research/judge/0"
...
def test_trace_then_capture_then_replay(tmp_path, monkeypatch):
    ...
    assert len(rows) == 2 and rows[0]["key"] == "analyst_large/research/0" and rows[0]["tools"]
    ...
    prof.write_text("\n".join(json.dumps(x) for x in [
        {"key": "analyst_large/research/0", "concurrency": 8, "ok": True, "ttft_ms": 400.0, "total_ms": 1200.0,
         "prompt_tokens": 2200, "completion_tokens": 90, "model": "m"},
        {"key": "analyst_large/research/0", "concurrency": 32, "ok": True, "ttft_ms": 900.0, "total_ms": 30.0,
         "prompt_tokens": 2300, "completion_tokens": 95, "model": "m"},
        {"key": "task_ticket/writing/0", "concurrency": 32, "ok": True, "ttft_ms": 100.0, "total_ms": 40.0,
    ...
    assert r.headers["x-mock-profile"] == "analyst_large/research/0"
...
def test_replay_parses_a_streamed_response(tmp_path):
    ...
    async def main():
        async with httpx.AsyncClient(transport=httpx.MockTransport(handler)) as client:
            item = {"key": "task_ticket/writing/0", "archetype": "task_ticket", "role": "writing", "phase": "0",
