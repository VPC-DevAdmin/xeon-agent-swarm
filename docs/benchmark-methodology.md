...
|---|---|---|---|---|---|---|---|
| Task agent | one ticket: one knowledge-base lookup, one record, the worker's answer handed back | 1 | 3 + 1 | 1 at depth 32 | about 1 core-s | 1,390 (2,440 in the v2.2 shape) | 0.7 |
| Research agent | 90 source pages fetched and parsed, nine retrievals at rerank depth 128 | 3 | 16 + 4 | 9 at depth 128 | about 25 core-s | 16,100 | 1.6 |
| Ingestion agent | 50 PDF pages rendered, OCR'd, redacted, chunked, embedded and indexed | 1 | 5 + 2 | none | about 100 core-s | 3,670 | 27 |
| Data analyst | three jobs over 100 million rows, the second over two periods | 3 | 13 + 4 | 2 at depth 64 | about 190 core-s | 13,380 | 14 |
| Code agent | setup, CI and verification over Lua 5.4.7 and SQLite 3.50.4 | 3 | 13 + 4 | 3 at depth 64 | about 150 core-s | 12,850 | 12 |
...
workload version and are read from its versioned query set
(`data/capacity/queryset/`), never assumed: two per ticket in v2.2, one
in v2.3. Every archetype that acts looks things up
...
- **Task agent**: a trigger, triage, or routing agent: born, does one
  ...
  of any deployment by count. Its 1,390 tokens are the model's for this
  shape (1,490 in the calibration): about 400 in the planner's
  delegation, 950 in the worker's two turns and 50 in the judgment,
  against 2,440 when the planner
  ...
  the record keeps one model for every archetype. Latency 17 s at every
  load, almost all of it model wait.
...
other. The allocation of record for the enterprise tile is 6 cores for
the reranker (two processes of three inference threads each), 1 for the
query embedder, 2 for the ingest embedder, and the remaining 55 for the
...
Provisioning selects it with `RERANK_PHYS_CORES=6 RERANK_WORKERS=2
RERANK_THREADS=3 EMBED_PHYS_CORES=1 INGEST_EMBED_PHYS_CORES=2`, and it
...
two processes keep a depth-128 query's wait short at that load; at
capacity the tiers run at 42%, 14% and 32%. The same tile ran on
8/2/3/51 (v2.2, section 12), whose tiers ran at 40%, 18% and 22% with
four cores' worth of headroom that the rebalance moved to the
application pool. Earlier allocations are listed with the superseded
results in section 12. Pinning is by cpuset,
...
enterprise tile's capacity the application pool's 55 cores are about
93% occupied over the steady window (time-averaged per core, busier
sibling) and past the cliff 98 to 100%, full on both threads of most
...
- **Allocation.** Cores are divided so that the tiers keep headroom past
  ...
  that is 6 + 1 + 2 cores for the tiers and 55 for everything else. A
  core moved from a tier with headroom to the application pool buys
  capacity at the same host work per token only while the pool is not
  yet in sibling contention: the four cores moved from the tiers at v2.3
  slowed the queue's growth at 0.90 workflows/s (from +50 to +44 over
  the hold) without making it stable, because the pool was already at
  98%. A tier sized below its pair budget turns every lookup into a
  queue wait.
...
`data/capacity/serving/gptoss20b-low-faithful-v23/calls.jsonl`, recorded
from the query set `data/capacity/queryset/enterprise-v23.jsonl` (the
v2.2 profile and query set, `gptoss20b-low-faithful` and
`enterprise.jsonl`, stay beside them for the superseded set); the
...
44 over the hold with the analyst a quarter slower again and the code
agent a sixth; the research and task agents, whose time is model wait,
hold 193 and 17 s at every rate. The cliff is the application pool's,
at 93% occupancy at capacity and 98% one rung above it with both
threads of most cores busy: under that contention each job's CPU time
rises by a third to a half over its stand-alone cost, which is what
makes the cliff sharp. The four cores the rebalance moved into the
pool slowed the queue's growth at 0.90 (from +50 to +44 over the hold
against the v2.2 allocation) without making it stable. The retrieval
...
At capacity one server, 85% busy, keeps 1.6 GPUs busy at the record
rate and 2.4 at the conservative 2,400. Stated per core, 9.6 core-ms
per token at 3,565 tokens/s is 34 cores per GPU, so a 64-core socket
run flat out would pair with 1.9 GPUs. That last figure is an
...
capacity rung (65 core-s, which the model does not change), and takes
...
|---|---|---|---|---|---|---|
| gpt-oss-20b, low reasoning (record) | 6,710 measured | 9.6 | 5,660 | 3,565 | 1.59 | 34 |
| gpt-oss-120b, low reasoning | 4,090 (0.61 of the record) | 15.8 | 3,450 | 1,450 (1,524 output tokens/s published at 50 concurrent, less 5%) | 2.4 | 23 |
| Qwen3.8 Flash, thinking off | 3,620 (0.54) | 17.9 | 3,060 | not cited; shown at 3,565 | 0.86 | 64 |
| gpt-oss-20b, medium reasoning | 14,090 (2.1) | 4.6 | 11,890 | 3,565 | 3.3 | 16 |
...
effect outweighs the first: for the 120B it does (2.4 GPUs), because its
...
serving rate and the ratio doubles with it (3.3 GPUs), which is the
...
- **Enterprise tile with the v2.2 task agent (8 September 2026).** The
  archetypes of section 2 except the task agent, which re-drafted its
  worker's answer in a closing planner turn and graded that synthesis
  with a second judgment (2,440 tokens and 30 s per ticket), on
  allocation 8/2/3/51. Capacity 0.84 workflows/s with 0.90 falling
  behind in every seed, 152 resident, 83% of the host busy over the
  steady window, 7,030 output tokens per workflow on the declared mix,
  5,920 tokens/s, 9.0 core-ms per token, 1.66 GPUs at 3,565 tokens/s.
  Sets `set-20260908-025329` (0.84, 0.96), `set-20260908-053752` (0.72),
  `set-20260908-115604` (0.90); photographs `photo-11401-20260908-070145`,
  `photo-11501-20260908-075234`, `photo-11601-20260908-084324` (152
  sessions, 150 in flight); evidence commits 5add991, bce6f26, 3df029a,
  325d7d7; profile `gptoss20b-low-faithful`, query set
  `enterprise.jsonl`; samples `set-11400-mpstat.log`,
  `set-11400c-mpstat.log`, `set-11400d-mpstat.log`.
