...
Writes <set>/summary.json and <set>/summary.md. The headline is the set's
capacity: the highest rate at which every series keeps up (joint 95/95
backlog steady, generator within 5% of the offered rate), with the median and range of the fleet-wide achieved rate and of
...
def main() -> None:
    ...
    for r in rates:
        ...
        backlog = ", ".join(f"{j['backlog_start']}->{j['backlog_end']}" for j in js)
        def lat(sid, q):
    ...
    lines += ["", "## Capacity (the highest rate at which every series keeps up: completions pace arrivals over the hold)", ""]
